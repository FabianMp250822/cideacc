...
export default function AboutPage() {
  ...
  const values = [
    { icon: <Star className="h-8 w-8 text-primary" />, text: t('about.values.list.0') },
    { icon: <Handshake className="h-8 w-8 text-primary" />, text: t('about.values.list.1') },
    { icon: <ShieldCheck className="h-8 w-8 text-primary" />, text: t('about.values.list.2') },
    { icon: <Award className="h-8 w-8 text-primary" />, text: t('about.values.list.3') },
    { icon: <BookOpen className="h-8 w-8 text-primary" />, text: t('about.values.list.4') },
  ];
  ...
  return (
    <div className="container mx-auto px-4 py-16 md:px-6 md:py-24 lg:py-32 animate-fade-in">
      ...
      <div className="mt-20 grid gap-16 lg:grid-cols-2 lg:gap-24">
        ...
        <div className="space-y-6">
          <h2 className="font-headline text-3xl font-bold text-foreground flex items-center gap-3"><TrendingUp className="text-primary"/>{t('about.vision.title')}</h2>
         ...
         <div className="container mx-auto">
            <div className="text-center">
              <h2 className="font-headline text-3xl font-bold text-foreground flex items-center justify-center gap-3">
                <BookOpen className="text-primary" /> {t('about.history.title')}
              </h2>
          ...
          {teamMembers.map((member) => (
            <Card key={member.name} className="text-center transition-all duration-300 hover:shadow-2xl hover:-translate-y-2">
              <CardContent className="flex flex-col items-center p-8">
                ...
                <p className="text-primary font-semibold">{t(`about.team.members.${member.role}`)}</p>
              </CardContent>
            </Card>
          ))}
        </div>
